 ...
 */
 ...
 Tools.connect = function () {
   ...
   function callingAlert() {
	 ...
	 console.log("userIds.includes(userId) ", userIds.includes(user));
	 if (!userIds.includes(user)) {
	   //call callAlert
	   checkHasAccess();
	   console.log("after calling checkhasacess ", hasAccess, user);
	   if (hasAccess) {
		 console.log("wow");
		 userIds.push(user);
		 ...
		 console.log("You have been denied", hasAccess);
	   }
	 }
 ...
 Tools.drawAndSend = function (data, tool) {
   ...
   Tools.socket.on("sendUser", (userId) => {
	 if (localStorage.getItem(Tools.boardName) == null) {
	    var userIds = [];
	   if (userIds.length == 0) {
		 console.log("isnide usersId.length ");
...
//    if(localStorage.getItem(Tools.boardName)!=null){
	console.log(
		"localStorage.getItem(Tools.boardName).includes(loggedUser) ",
		localStorage.getItem(Tools.boardName).includes(loggedUser),
		loggedUser
	  );
	  Tools.socket.emit(
